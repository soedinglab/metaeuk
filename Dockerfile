...
RUN dpkg --add-architecture $TARGETARCH \
    && apt-get update \
    && apt-get install -y \
      build-essential cmake xxd git curl \
...
RUN if [ "$TARGETARCH" = "arm64" ]; then \
      ...
      CC=aarch64-linux-gnu-gcc CXX=aarch64-linux-gnu-g++ cmake -DHAVE_ARM8=1 -DHAVE_MPI=0 -DHAVE_TESTS=0 -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=. -DRust_CARGO_TARGET=aarch64-unknown-linux-gnu ..; \
      make -j $(nproc --all); \
      ...
      cmake -DHAVE_SSE2=1 -DHAVE_MPI=0 -DHAVE_TESTS=0 -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=. -DRust_CARGO_TARGET=x86_64-unknown-linux-gnu ..; \
      make -j $(nproc --all); \
      mv src/${APP} /opt/build/${APP}_sse2; \
      cd /opt/build/build_sse41; \
      cmake -DHAVE_SSE4_1=1 -DHAVE_MPI=0 -DHAVE_TESTS=0 -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=. -DRust_CARGO_TARGET=x86_64-unknown-linux-gnu ..; \
      make -j $(nproc --all); \
      mv src/${APP} /opt/build/${APP}_sse41; \
      cd /opt/build/build_avx2; \
      cmake -DHAVE_AVX2=1 -DHAVE_MPI=0 -DHAVE_TESTS=0 -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=. -DRust_CARGO_TARGET=x86_64-unknown-linux-gnu ..; \
      make -j $(nproc --all); \

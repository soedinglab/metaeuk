...
if notExists "${TAX_ASSIGNMENT_BASENAME}_tax_per_pred.tsv"; then
    # shellcheck disable=SC2086
    "$MMSEQS" createtsv "${TMP_PATH}/preds" "${TAX_TARGET_DB}" "${TMP_PATH}/tax_per_pred" "${TAX_ASSIGNMENT_BASENAME}_tax_per_pred.tsv" \
...
if notExists "${TAX_ASSIGNMENT_BASENAME}_tax_per_contig.tsv"; then
    # shellcheck disable=SC2086
    "$MMSEQS" createtsv "${CONTIGS_DB}" "${TAX_TARGET_DB}" "${TMP_PATH}/tax_per_contig" "${TAX_ASSIGNMENT_BASENAME}_tax_per_contig.tsv" \
